...
def activities(subject_id):
    ...
    if request.method=='POST':
        if current_user.role=='direction': abort(403)
        db.session.add(Activity(subject_id=sub.id,title=request.form['title'],component=request.form['component'],max_points=float(request.form['max_points']))); db.session.commit(); return redirect(url_for('activities',subject_id=sub.id))
